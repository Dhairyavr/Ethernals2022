...
contract EthQF is ERC20,ERC2771Context {
   ...
   function createProject (string memory _ipfsHash,uint  _no_of_installments,string memory _date,string memory _amountToRaise)  public {
       uint [] memory _installment_ids;
       Project memory proj= Project(_msgSender(),_ipfsHash,0,0,_no_of_installments,_no_of_installments,_installment_ids, _date,0,_amountToRaise );
       ...
       project_id+=1;
        ...
        projectCollection[_msgSender()].push(proj);
        projectIdCollection[_msgSender()].push(project_id-1);

   }
   
   function enrollSponsor(uint _round_id,address _sponsor,uint _amount) payable public {
       
       Sponsor memory sp=Sponsor(_msgSender(),_amount);
...
// call projectSqrtAmount to get old sum of square roots , add square root of current Contribution to it and take a square of that amount & pass it as sq_amount 
    ...
    function makeContribution(uint _project_id,uint _amount,uint sq_amount,uint _sqrtAmount,string memory _date) payable public { 

        require(_project_id < project_id,"Invalid Project Id ");
        
        emit Contribution(_msgSender(),_amount,_project_id,_date);
        projects[_project_id].crowdfundAmount+=_amount;
        projects[_project_id].sqrtAmount+=_sqrtAmount;
        
        if(check_contributed_to[_project_id][_msgSender()]==false) {
            
            contributedTo[_project_id].push(_msgSender());
            check_contributed_to[_project_id][_msgSender()]=true;
            _mint(_msgSender(),projects[_project_id].no_of_installments* (10**18) );
...
//_quadratic_funding_amount = (crowdfundAmount + (  (project_sum_amount[_project_id]/total_amount)*liquidity_pool_amount ) ) / projects[_project_id].no_of_installments     
    ...
    function retrieveFirstInstallment(uint _project_id,uint _quadratic_funding_amount) public {
        
        require(_msgSender()==projects[_project_id].owner,"Invalid Owner");
        //require(projects[_project_id].projStatus==true,"Project is closed");
        
        projects[_project_id].remaining_installments-=1;
        // projects[_project_id].installment_ids.push(installment_id);
        // installment_id+=1;
...
//this function is called everytime(except for 1st installment) to get Installment amount after voting is done in favour of yes 
    ...
    function retrieveInstallment (uint _project_id) public {
...
// this function will be called for all installments except 1st Installment
//It will initiate Voting on next Installment which is to be rolled out
    
    // function startInstallmentVoting(uint _project_id,string memory _date) public { 
...
//Vote 0 = No and 1 = Yes
    ...
    function castVote(uint _project_id,uint vote) public { 

        uint length=projects[_project_id].installment_ids.length;
        uint id=projects[_project_id].installment_ids[length-1];
        
        require(contributorVotingStatus[_project_id][id][_msgSender()]==false,"Voter has already voted in this Installment");
        ...
        contributorVotingStatus[_project_id][id][_msgSender()]=true;
        _burn(_msgSender(),1*(10**18));
        
    }

        function castFirstVote(uint _project_id,uint vote,string memory _date) public { 
