...
exports.SetRoundDate = async (req, res) => {
  ...
  for (let index = 1; index <= 7; index++) {
    if (index <= 3) {
      ...
      x.setDate(x.getDate() + 50); //50
      //1 day in sec=1*86400
      // let time = 35 * parseInt(index) * 60 ;
      let time = 53 * 86400 * parseInt(index);
      ...
      let time1 = 59 * 86400 * parseInt(index);
    ...
    if (index <= 5) {
      ...
      x.setDate(x.getDate() + 40); //40
      // let time = 25 * parseInt(index) * 60;
      let time = 43 * 86400 * parseInt(index);
      await redis_get.setex(
      ...
      let time1 = 49 * 86400 * parseInt(index);
    ...
    if (index <= 7) {
      ...
      x.setDate(x.getDate() + 30); //30
      // let time = 15 * index * 60;
      let time = 33 * 86400 * parseInt(index);
      await redis_get.setex(
      ...
      let time1 = 39 * 86400 * parseInt(index);
